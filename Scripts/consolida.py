...
class Consolida():
    ...
    def main(self):
        ...
        for cont in contenidos:
            count += 1
            print(count)
            dicc = {'Area'                    : self.getAreas(category, areas, cont['Titulo']),
                    'Pais'                    : cont['Pais'],
                    'Ciudad'                  : cont['Ciudad'],
                    'Calles'                  : self.getCalles(cont['Universidad'], universidad_whed, calle_whed),
                    'Institucion'             : self.getInst(cont['Universidad'], universidad_whed, institucion_whed),
                    'Idioma'                  : self.getLang(cont['Universidad'],   universidad_whed, lenguaje_whed),
                    'Subtitulos'              : None,
                    'Calificacion'            : cont['Calificacion'],
                    'Nivel'                   : cont['Nivel'],
                    'Part Time'               : self.getCleanYear(cont['Duracion Parcial']),
                    'Full Time'               : self.getCleanYear(cont['Duracion Full']),
                    'Cursada'                 : cont['Cursada'],
                    'Duracion'                : self.getCleanYear(self.getDuracion(cont['Duracion Parcial'], cont['Duracion Full'])),
                    'Titulo'                  : cont['Titulo'],
                    'Universidad'             : cont['Universidad'],
                    'Descripcion Programa'    : cont['Descripcion'].replace('&nbsp;', ' '),
                    'Descripcion Universidad' :self. getDesc(cont['Universidad'], universidad_whed, universidad_desc_whed),
                    'Categoria'               : self.categoryUnique([category[i] for i in range(len(category)) if category[i] in cont['Titulo']]),
                    'Deadline'                : None,
                    'Requerimientos'          : None,
                    'Tutition Free'           : self.getMatricula(cont['Matricula'],  cont['Moneda Matricula'] , cont['Unidad Matricula'] ),
                    'Valoracones'             : None,
                    'Link University'         : self.getUniversity(cont['Universidad'],universidad_whed ,link_universidad_whed ),
                    'logo'                    : cont['Logo'],
                    'Multimedia'              : None
                    }
    ...
    def getUniversity(self, universidad, universidad_whed, link_universidad_whed):
        if universidad != None:
    ...
    def getAreas(self, category, areas, categoria):
        areas_totales = []
        try:
    ...
    def getDuracion(self, datapart, datafull):
        data_part = datapart
        data_full = datafull
        if data_part == None:
            return data_full
    ...
    def getMatricula(self, importe, moneda, unidad):
    ...
    def getLang(self, universidad, universidad_whed, lenguaje_whed):
        if universidad != None:
            universidad = universidad.lower()
            for i in range(len(universidad_whed)):
                try:
    ...
    def getCalles(self, universidad, universidad_whed, calle_whed):
        if universidad != None:
            universidad = universidad.lower()
            for i in range(len(universidad_whed)):
                try:
    ...
    def getInst(self, universidad, universidad_whed, institucion_whed):
        if universidad != None:
            universidad = universidad.lower()
            for i in range(len(universidad_whed)):
                try:
    ...
    def getDesc(self, universidad, universidad_whed, universidad_desc_whed):
        if universidad != None:
            universidad = universidad.lower()
            for i in range(len(universidad_whed)):
                try:
